...
- Durbin-Watson statistic helpers and exact Imhof-based p-values with SwiftyBoost quadrature integration, plus Swift Testing coverage for the quadratic form CDF and DW convenience API.
...
- SwiftUI-based `TimeSeriesPlotter` to visualise time series with smoothing overlays and autocorrelation bars, offering macOS window presentation and PNG export.
...
- TimeSeries Durbin-Watson Swift Testing coverage including symmetric lambda stress cases and longer residual sequences.
...
- DurbinWatson.swift now derives DW eigenvalues from a stable residual-maker projection and offers a `pValues(residuals:designMatrix:)` overload to align p-values with dwtest.
- DurbinWatson.swift now accepts raw time series, computes OLS residuals (intercept or intercept+trend), and returns DW statistics with exact p-values without external preprocessing.
- Added `Sources/SwiftyStats/TimeSeries/DurbinWatson2.swift`, implementing Farebrother AS 204 for exact DW p-values with an Accelerate-backed projection/eigen pipeline and raw-series convenience entry points.
- R script `R-Scripts/TukeyKramerReference.R` to compute Tukey-Kramer post-hoc comparisons for a fixed eleven-group dataset.

### Changed
- Updated `KS-OPT.md` to reflect the latest KS implementation review findings.
- Updated `SwiftyStats-Manual.md` and `Using-MLE.md` to reflect current hypothesis-testing namespaces and KS bootstrap helpers, including an MLE-based KS validation example.
- Updated `README.md` with SwiftPM version-based dependency setup and a brief `0.1.0` release note.
...
- Translated `TimeSeries/DurbinWatsonClaude.swift` documentation and validation output to English and standardised comments to ASCII.
...
- DocC article metadata now uses `@DisplayName` to avoid unexpected-content warnings during documentation builds.
...
- Corrected `TimeSeries/DurbinWatsonClaude.swift` to use the proper Imhof/Davies integrand (with the `2λu` scaling) and a symmetric Householder tridiagonal step so DW eigenvalues and exact p-values match the intended distribution.
...
- DurbinWatson.swift now computes the upper tail via the `4 - d` symmetry instead of using a complement, matching dwtest-style p-values.
- DurbinWatsonClaude.swift centers the trend regressor, symmetrizes MAM before eigendecomposition, and offers a raw-series DW entry point so projections and p-values align with dwtest outputs.
- DurbinWatsonClaude.swift now uses Accelerate (QR + BLAS/LAPACK) to form residual projections and eigenvalues for stable DW p-values that match `dwtest`.
- DurbinWatsonClaude.swift switches the DW p-value calculation to the Pan/Farebrother-style positive quadratic form with 4−d symmetry for the upper tail, matching R's dwtest tail behavior more closely.
...
## [2.0.0] - 2025-11-22
### Added
- DocC articles `HowToUseSwiftyStats` and `DocHosting` covering the end-to-end workflow and GitHub Pages publishing steps for the static documentation site.
...
- DocC catalog (`Sources/SwiftyStats/SwiftyStats.docc`) that organises every public function, property, enum, and supporting struct into semantic articles for ``SSExamine``, ``Inferential``, and shared utilities.
...
- Structured user manual (`SwiftyStats-Manual.md`) that explains how to call every public function, property, enum, and struct across SSExamine, Inferential, MLE, RNG, and shared utilities.
...
- Exhaustive `AGENTS.md` appendix that lists the file path and line number for every enum, struct, class, and function defined in the repository.
...
### Changed
- Restructured the ``SSExamine`` DocC accessors article into semantic sections (file management, counts, location, dispersion/scale, shape, robustness, GOF) for faster navigation.
...
- Documented the `isGaussian`/`testForDistribution` helpers and refreshed README + SwiftyStats-Manual with a cheat sheet for the 2.0.0 release and GitHub Pages hosting notes.
...
- Added DocC-style summaries for every remaining public declaration spanning GOF tests, Nelder–Mead options, RNG helpers, score functions, and SSExamine accessors so symbol documentation is now exhaustive.
...
- Rebuilt `SwiftyStats-Manual.md` into a scenario-driven manual that walks through installation, SSExamine workflows, inferential helpers, MLE usage, CLI demos, and troubleshooting guidance.
